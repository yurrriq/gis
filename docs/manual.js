...
function overwriteStyle() {
  ...
  if (style == 0) style = valueString(document.cookie, "GAPDocStyle");
  if (style == 0) return;
  if (style == "default") deleteCookie("GAPDocStyle");
  else {
    ...
    */
    ...
    for (var i = 0; i < stlist.length; i++) {
      ...
      document.writeln(
        '<script src="' + stlist[i] + '.js" type="text/javascript"></script>',
      );
    }
  }
}
...
function jscontent() {
  for (var i = 0; i < jscontentfuncs.length; i++) jscontentfuncs[i]();
}
